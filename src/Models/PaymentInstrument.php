...
class PaymentInstrument extends ModelsBase
{
    ...
    {
        if (!is_array($this->data['SupportedCurrencies'])) {
    ...
    {
        if (!is_array($this->data['AdditionalRequirements'])) {
    ...
    public function toArray($canonize = false)
    {
        return [
            'AdditionalRequirements' => is_array($this->data['AdditionalRequirements'])
                ? array_map(fn($additionalRequirement) => $additionalRequirement->toArray($canonize), $this->data['AdditionalRequirements'])
                : [],
            'AnonInstrumentUsageTimeLimit' => $this->data['AnonInstrumentUsageTimeLimit'],
            'CreditLimits' => is_array($this->data['CreditLimits'])
                ? array_map(fn($currency) => $currency->toArray($canonize), $this->data['CreditLimits'])
                : [],
            ...
            'Issuer' => $this->data['Issuer'] !== null ? $this->data['Issuer']->toArray($canonize) : null,
            'Name' => $this->data['Name'],
            'SessionCreationId' => $this->data['SessionCreationId'],
            'Status' => $this->data['Status'],
            'SupportedCurrencies' => is_array($this->data['SupportedCurrencies'])
                ? array_map(fn($currency) => $currency->toArray($canonize), $this->data['SupportedCurrencies'])
                : [],
