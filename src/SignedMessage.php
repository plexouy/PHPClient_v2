...
class SignedMessage implements \Stringable
{
     ...
     */
    public function __construct(protected $object = null, protected $fingerprint = null, protected $utcUnixTimeExpiration = null, protected $signature = null)
    {
    }

    public function __set($name, $value)
    {
        $method_name = 'set' . ucfirst((string) $name);
    ...
    public function toArray($canonize = false)
    {
        $arr = [
            'Object' => [
                'Fingerprint' => strtoupper((string) $this->fingerprint),
...
//        $message['Object'] = Utilities\functions\array_filter_recursive($message['Object']);
        ...
        $json = preg_replace_callback('/":"float\((\d+)(\.(\d+))?\)"/', fn($matches) => sprintf('":%d.%s', $matches[1], (array_key_exists(3, $matches) ? $matches[3] : 0)), $json);
        $json = str_replace('{"__sdkEmptyMap":1}', '{}', $json);
        return $json;
    }
    ...
    public function sign($cert, $expirationTime = null)
    {
        ...
        if ($expirationTime || is_null($this->utcUnixTimeExpiration)) {
            $this->setExpirationTime(($expirationTime ?: time() + 600));
        }
        ...
        if (!openssl_sign($base_string, $signature, $cert->pkey, OPENSSL_ALGO_SHA512)) {
            ...
            throw new Exception\PlexoException(($errstr ?: 'No fue posible firmar la petición.'));
        }
        $this->signature = base64_encode((string) $signature);
    }

    public function verify($cert)
    {
        $base_string = $this->getSignatureBaseString();
        return openssl_verify($base_string, base64_decode((string) $this->signature), $cert->cert, OPENSSL_ALGO_SHA512);
    }
    ...
    public function validate()
    {
        ...
        if (!$cert) {
            ...
            $cert = Certificate\Certificate::fromServerPublicKey($response['Key'], null);
        }
        if (!$this->verify($cert)) {
